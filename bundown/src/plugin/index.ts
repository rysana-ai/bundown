    ...
    async setup (build) {
      ...
      build.onLoad({ filter: /\.(sh|bash|py|go)$/ }, async (args) => {
        ...
        if (Bun.main === args.path) {
          ...
          const runtimeArg = parsed.values['bundown-runtime'] || parsed.values['brrrrr'] || parsed.values['brrr']
          ...
          // Execute the file.
          const { exitCode } = await runtime.call({ runtime: runtimeResolved, path: args.path, args: [] }) 
