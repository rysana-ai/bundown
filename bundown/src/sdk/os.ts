...
export type OperatingSystem =
  | { type: 'friendly-architecture', friendly: FriendlyOperatingSystem, architecture: NodeJS.Architecture }
...
export const os = {
  ...
  platforms: [ 'aix', 'android', 'darwin', 'freebsd', 'haiku', 'linux', 'openbsd', 'sunos', 'win32', 'cygwin', 'netbsd' ] as const satisfies NodeJS.Platform[],
  architectures: [ 'arm', 'arm64', 'ia32', 'mips', 'mipsel', 'ppc', 'ppc64', 'riscv64', 's390', 's390x', 'x64' ] as const satisfies NodeJS.Architecture[],
  platform,
  architecture,
  matches: {
    os: (operatingSystem: OperatingSystem): boolean => {
      switch (operatingSystem.type) {
        case 'friendly-architecture':
          return os.matches.os({ type: 'friendly', friendly: operatingSystem.friendly }) &&
                 os.matches.os({ type: 'architecture', architecture: operatingSystem.architecture })
        case 'platform-architecture':
          return os.matches.os({ type: 'platform', platform: operatingSystem.platform }) &&
                 os.matches.os({ type: 'architecture', architecture: operatingSystem.architecture }) 
        case 'friendly':
          return (operatingSystem.friendly === 'linux' && os.platform === 'linux') ||
    ...
    name: (name?: string): OperatingSystem | undefined => {
      ...
      if (name.includes('-')) {
        ...
        if (friendlyOrPlatform?.type === 'friendly') {
          return { type: 'friendly-architecture', friendly: friendlyOrPlatform.friendly, architecture: architecture.architecture }
        }
        if (friendlyOrPlatform?.type === 'platform') {
          return { type: 'platform-architecture', platform: friendlyOrPlatform.platform, architecture: architecture.architecture }
        }
        return undefined
      }
      ...
      if (os.architectures.includes(lowercase as (typeof os.architectures)[number])) {
        return { type: 'architecture', architecture: lowercase as (typeof os.architectures)[number] }
      }
    }
  },
  has: {
